...
#定义tensorboard的输出空间
writer = SummaryWriter(log_dir='data_log_16_Batchnorm_1000_32',comment='data recoard for lipreading',flush_secs=120)
#定义训练和测试函数
def train_test_fun(
        model:torch.nn.Module,
        train_dataloader_rgb:DataLoader,
        train_dataloader_of:DataLoader,
        test_dataloader_rgb:DataLoader,
        test_dataloader_of:DataLoader,
        loss_fn:nn.Module,
    ...
    for epoch in range(Epoch):
        ...
        if epoch % 5 == 0 and test:
            with torch.no_grad():
                ...
                model.eval()
                print(f'~~~~~~~~~~~~~~模型开始测试~~~~~~~~~~~~~~~~~~~~')
                for id, data in tqdm(enumerate((zip(test_dataloader_rgb, test_dataloader_of)))):
                    (X, y), (M, n) = data
                    test_loss = 0
                    X, y = X.to(device), y.to(device)
                    M, n = M.to(device), n.to(device)
